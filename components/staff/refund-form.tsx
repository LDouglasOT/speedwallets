...
export function RefundForm() {
  ...
  const handleRefund = async () => {
    ...
    try {
      const response = await fetch('/api/staff/refund', {
        method: 'POST',
      ...
      setSuccess(`Refund of UGX ${amountNum.toLocaleString()} processed successfully!`)
    ...
    <div className="grid gap-6 md:grid-cols-2">
      ...
      <Card className={student ? '' : 'opacity-50'}>
        <CardHeader>
          ...
          {student && (
            <CardDescription>
              Current Balance: UGX {student.balanceUgx.toLocaleString()}
            </CardDescription>
          )}
        </CardHeader>
        <CardContent>
          {!student ? (
            <p className="text-center text-muted-foreground py-8">
              Search for a student to process a refund
            </p>
          ) : (
            <FieldGroup>
              <Field>
                <FieldLabel>Refund Amount (UGX)</FieldLabel>
                <Input
                  type="number"
              ...
              <Field>
                ...
                <Input
                  type="text"
                  placeholder="e.g., Item not available, Wrong order"
                  value={description}
